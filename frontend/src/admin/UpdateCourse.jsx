...
function UpdateCourse() {
	...
	useEffect(() => {
		const fetchCoursesData = async () => {
			...
			try {
				...
				console.log(error, "Failed to fetch Course Data");
	...
	const handleUpdateCourse = async (e) => {
		...
		try {
			...
			toast.success(response.data.message || "Front: Course Updated Successfully");
			...
			console.log(error, "Front: Failed to Updated Course");
			toast.error(error.response?.data?.errors || error.response.data?.message || "Front: Failed to Updated Course");
		}
	};
		...
		<div>
			<div className="min-h-screen py-10">
				<div className="max-w-4xl mx-auto p-6 border rounded-lg shadow-lg">
					<div className="mb-4">
						<Link className="bg-orange-400 py-2 px-4 rounded-lg text-white hover:bg-orange-950 duration-300" to={"/admin/our-courses"}>
							Go to Courses
						</Link>
					</div>
					...
					<form onSubmit={handleUpdateCourse} className="space-y-6">
						...
						<div className="space-y-2">
							...
							<input type="number" placeholder="Enter your course price" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full px-3 py-2 border border-gray-400 rounded-md outline-none" />
						</div>

						<div className="space-y-2">
							<label className="block text-lg">Course Image</label>
							<div className="flex items-center justify-center">
								<img src={imagePreview ? `${imagePreview}` : "/imgPL.webp"} alt={`Course: ${title}`} className="w-full max-w-sm h-auto rounded-md object-cover" />
							</div>
