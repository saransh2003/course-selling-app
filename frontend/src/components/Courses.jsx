...
function Courses() {
	...
	console.log("In courses Section: ", courses);

	useEffect(() => {
		// User and Token Section
		const user = JSON.parse(localStorage.getItem("user"));
		// console.log("User (Courses): ", user);
		// console.log("Token (Courses): ", user?.token);
	...
	return (
		<div className="flex">
			...
			{/* Sidebar */}
			<aside className={`fixed top-0 left-0 h-screen bg-gray-100 w-64 p-5 transform z-10 transition-transform duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:static`}>
				...
				<div className="overflow-y-auto h-[75vh]">
					{loading ? (
						...
						<div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
							{courses.map((course) => (
								<div key={course._id} className="flex flex-col  align-start border border-gray-200 rounded-lg p-4 shadow-sm">
									...
									<Link
										...
										className="bg-orange-500 w-24 text-white px-4 py-2 rounded-lg hover:bg-blue-900 duration-300"
									>
										Buy Now
									</Link>
								</div>
